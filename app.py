...
def create_user(name, email, password):
    hashed_pw = incryption.hashpw(password.encode('utf-8'), incryption.gensalt())
    if users_db.find_one({'email':email}):
        st.write("Error! User Already Exist")
    else:
        users_db.insert_one({
            "name":name,
            "email":email,
...
def login_user(email, password):
    ...
    if user and incryption.checkpw(password.encode('utf-8'), user["password"]):
...
def login_page():
    ...
    st.button("Reset here", on_click=lambda: st.session_state.update({"page": "Reset"}))

    st.write("Don't Have an Account ? ")
    st.button("Signup for a new account", on_click=lambda: st.session_state.update({"page": "Signup"}))
    ...
    if st.button("Reset Password"):
        if not email or not new_password or not confirm_password:
            ...
            if not user:
                ...
                hashed_pw = incryption.hashpw(new_password.encode("utf-8"), incryption.gensalt())
...
#-----------------SignUp Page -----------------------
def signup_page():
    st.title("SignUp For GoodPrompt")
    name = st.text_input("What should we call you ?")
    email = st.text_input("Whats your email ?")
...
def main_page():
    st.title("Welcome To GoodPrompt")
    user = st.session_state.username
    name = users_db.find_one({"email":user})
    display_name = name['name']
    st.title(f"Hello {display_name}")

    user_request = st.text_area("Whats do you want to generate the prompt for?")


    st.sidebar.header("Optional Params")
    user_tone = st.sidebar.text_input("Whats should be the tone?")
    context = st.sidebar.text_input("Context")
    prompt_size = st.sidebar.text_input("Output Prompt Size")
    negative_aspects = st.sidebar.text_input("Tell Prompt what not to do (e.g. make assumptions, be funny or casual etc)")
    CountryLang = st.sidebar.text_input("Is the prompt related to a specific country and language (e.g. US and English, all countries & any language”?")
    industry = st.sidebar.text_input("Is this topic about a specific Industry? (e.g. oil & gas, Government policy, toys etc.)")
    outputFormat = st.sidebar.text_input("What does the generated prompt generate? Is it text, image or image link etc.")
    ...
    if st.button("Generate Prompt"):
        ...
        if CountryLang: optional_params.append(f"Country/Language:{CountryLang}")
        if industry: optional_params.append(f"Industry: {industry}")
        if outputFormat: optional_params.append(f"Output Format: {outputFormat}")
